...
#include <startup_msgs/StartUpSrv.h>
...
class Ekf
{
  ...
  private:
    // ekf
    void predict_diff_ekf(double v, double w, double dt, Eigen::Matrix3d odom_sigma);
    void predict_diff_ucekf(double v, double w, double dt);
    void predict_omni_ekf(double v_x, double v_y, double w, double dt, Eigen::Matrix3d odom_sigma);
    void predict_omni_ucekf(double v_x, double v_y, double w, double dt);
    ...
    std_srvs::Empty scan_srv;
    bool ready_signal_active;

    //fast spin srv
    ros::ServiceServer fast_spin_server;
    bool fast_spin(std_srvs::SetBool::Request &req, std_srvs::SetBool::Response &res);
    bool is_spinning;
