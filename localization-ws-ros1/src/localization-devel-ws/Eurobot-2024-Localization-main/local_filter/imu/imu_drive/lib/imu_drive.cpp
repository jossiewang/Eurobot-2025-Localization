    ...
    if(p_active_ != prev_active) {
        ...
        if (p_active_) {
            ...
            if(this->p_sub_from_nav_){
                ...
                this->vel_sub_ = nh_.subscribe("local_filter", 10, &IMU::VelocityCallback, this);
            }
...
void IMU::P_VelocityCallback(const nav_msgs::Odometry::ConstPtr &msg){

    double slope[3];
    double slope_accel[3];

    /* Rotation */
    slope[0] = abs(msg->twist.twist.angular.x) * p_cov_multi_vel_;
    slope[1] = abs(msg->twist.twist.angular.y) * p_cov_multi_vel_;
    slope[2] = abs(msg->twist.twist.angular.z) * p_cov_multi_vel_;

    /* Linear */
    slope_accel[0] = abs(msg->twist.twist.linear.x) * p_cov_multi_acl_;
    slope_accel[1] = abs(msg->twist.twist.linear.y) * p_cov_multi_acl_;
    slope_accel[2] = abs(msg->twist.twist.linear.z) * p_cov_multi_acl_;

    /* imu_output_ = slope * x + original_covariance */
    this->imu_output_.orientation_covariance[0] = 0.1;
    this->imu_output_.orientation_covariance[4] = 0.1;
    this->imu_output_.orientation_covariance[8] = 0.1;

    this->imu_output_.angular_velocity_covariance[0] = std::max(0.00000001, slope[0] + this->imu_output_backup_.angular_velocity_covariance[0]);
    this->imu_output_.angular_velocity_covariance[4] = std::max(0.00000001, slope[1] + this->imu_output_backup_.angular_velocity_covariance[4]);
    this->imu_output_.angular_velocity_covariance[8] = std::max(0.00000001, slope[2] + this->imu_output_backup_.angular_velocity_covariance[8]);

    this->imu_output_.linear_acceleration_covariance[0] = std::max(0.00000001, slope_accel[0] + this->imu_output_backup_.linear_acceleration_covariance[0]);
    this->imu_output_.linear_acceleration_covariance[4] = std::max(0.00000001, slope_accel[1] + this->imu_output_backup_.linear_acceleration_covariance[4]);
	this->imu_output_.linear_acceleration_covariance[8] = std::max(0.00000001, slope_accel[2] + this->imu_output_backup_.linear_acceleration_covariance[8]);

}

void IMU::VelocityCallback(const nav_msgs::Odometry::ConstPtr &msg){
